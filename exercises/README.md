...
├─ story_quiz.ex.js   INTERACTIVE סִפּוּר וְשְׁאֵלָה — READING comprehension: a short vowelled story (≤4 lines, age-7; topics חלל / חדי קרן / דינוזאורים / נסיכות — TEXT ONLY, no topic emoji on the card: a picture clue lets her answer without reading) + ONE multiple-choice question with vowelled answers; tap to SELECT, press the ✓ to SUBMIT (only then judged; wrong → penalty + re-pick). One of the SIX reading kinds sharing the ONE-PER-4 reading slots in Superman (sup) + אַלּוּפָה (mulc) — deck slots 4/8/12/16/20; there are only `READING_SLOTS`=5 slots per deck, so the six kinds ROTATE across games (see problems.js `_readingCards`).
├─ cloze.ex.js        INTERACTIVE הַשְׁלֵם אֶת הַמִּלָּה — a vowelled sentence with a dashed GAP + 3 vowelled word options; select → ✓ submits; the correct word DROPS into the gap. 36-sentence bank, no-repeat rotation. Shares the reading slots.
├─ true_false.ex.js   INTERACTIVE נָכוֹן אוֹ לֹא נָכוֹן — a 2-line vowelled mini-story + a STATEMENT that flips one detail (or not); select נכון/לא נכון → ✓ submits. 36-item bank, no-repeat rotation. Shares the reading slots.
├─ word_match.ex.js   INTERACTIVE הַתְאֵם מִלָּה לִתְמוּנָה — 3 emoji picture cards + 3 vowelled word pills; DRAG a word onto its picture (pointer-drag + ghost, like compare) or tap-word-then-tap-picture; correct locks green, wrong shakes + penalty; all 3 → solved. 42-pair bank, 3 popped per card. Shares the reading slots.
├─ sent_order.ex.js   INTERACTIVE סַדֵּר אֶת הַמִּשְׁפָּט — a 4-5 word vowelled sentence served SCRAMBLED as pills; tap words in order (Duolingo-style; tap a placed word to send it back), ✓ submits; wrong keeps the words for fixing. 38-sentence bank, no-repeat rotation. Shares the reading slots.
├─ rhyme.ex.js        INTERACTIVE חָרוּז — PHONOLOGICAL awareness (the one language kind that is NOT reading-for-meaning): a cue word + picture, and 3 picture+word options of which EXACTLY ONE rhymes; select → ✓ submits. A mistake reveals the SOUND AID naming the ending ("כַּד נִגְמֶרֶת בַּצְּלִיל אַד"). 33-pair bank (either member may be the cue) + a separate 20-word distractor pool filtered by the normalized final PHONEME (ך and ח are one sound), so a distractor can never rhyme. Shares the reading slots.
...
| `story_quiz.ex.js`| `TSQ`        | `'story','sup','mulc'`  | Interactive **READING comprehension** — a short vowelled story (≤4 lines, age-7) on חלל/חדי קרן/דינוזאורים/נסיכות + ONE multiple-choice question with vowelled answers (options shuffled per card; `{t:TSQ,topic,emoji,lines,q,opts,a}`, `a` = 1-based correct option → num1). The card is **TEXT ONLY** — the topic emoji is deliberately NOT rendered (a 🚀 over a space story let the child match the picture to the "חֲלָלִית" option and answer without reading); `emoji` survives only in the end-of-set report row, and `test_story_quiz_card_shows_no_emoji_picture_clue` guards story+question+options against any pictographic character. TAP selects (highlight), the module's **✓ submits** — only then judged: correct → `api.solved()`, wrong → `api.wrong(pick)` + re-pick; an empty submit only nudges. `make('sup'/'mulc')` → **1** (the reading slots are shared between the six reading kinds; topics AND stories rotate via NO-REPEAT shuffled queues); `make('story')` → the whole **48-story** library (12 per topic). Woven at **ONE PER 4 EXERCISES** (deck slots 4/8/12/16/20 — `_readingCards()` in problems.js fills `READING_SLOTS`=5 slots with 5 DISTINCT kinds, ROTATING through the 6 via `_rkCursor` so every kind gets its turn across games; base capped to 15 by `_capPool`). `aidsReveal:'always'` (a reading card — no arithmetic aid). |
| `cloze.ex.js`    | `TCZ`         | `'clz','sup','mulc'`    | Interactive **cloze** (הַשְׁלֵם אֶת הַמִּלָּה): a vowelled sentence with a dashed gap + 3 vowelled options (`{t:TCZ,pre,post,opts,a}`); select → ✓ submits, the correct word DROPS into the gap. 36-sentence bank, NO-REPEAT rotation; `make('sup'/'mulc')` → 1, `make('clz')` → the bank. `aidsReveal:'always'`. |
| `true_false.ex.js`| `TTF`        | `'tf','sup','mulc'`     | Interactive **true/false** (נָכוֹן אוֹ לֹא): a 2-line vowelled mini-story + a statement (`{t:TTF,lines,stmt,a}` — a=1 נכון / a=2 לא נכון); select → ✓ submits. False statements flip exactly ONE story detail. 36-item bank, NO-REPEAT rotation; `make('sup'/'mulc')` → 1, `make('tf')` → the bank. `aidsReveal:'always'`. |
| `word_match.ex.js`| `TWM`        | `'wm','sup','mulc'`     | Interactive **word↔picture matching** (הַתְאֵם מִלָּה לִתְמוּנָה): 3 emoji cards + 3 vowelled word pills (`{t:TWM,pairs,a:3}`); DRAG a word onto a picture (compare-style pointer-drag + ghost) or tap-word-then-tap-picture; correct locks green, wrong shakes + `api.wrong`; all 3 → `api.solved()`. 42-pair bank, 3 popped per card (no-repeat until the bank cycles); `make('sup'/'mulc')` → 1, `make('wm')` → 4 cards. `aidsReveal:'always'`. **Ghost lifecycle** (v9.68 fix): the `.wm-ghost` is `position:fixed; z-index:9999`, so one that outlives its drag floats over the success screen — every exit funnels through `endDrag()`, the drag is pinned to its `pointerId`, `pointercancel`/`blur` abort it, a 2nd pointer is rejected while a drag is live, and `killGhost()` sweeps by class so an orphan still dies. **Grab-anchored** (v9.75): a `pointerdown` records the press offset from the pill's centre (`ox/oy`) and the ghost is placed at `finger − offset`, so the grabbed point stays under the finger (no jump-to-centre on tablet). |
| `sent_order.ex.js`| `TSO`        | `'so','sup','mulc'`     | Interactive **sentence ordering** (סַדֵּר אֶת הַמִּשְׁפָּט): a 4-5 word vowelled sentence served SCRAMBLED (`{t:TSO,words,scr,a}`; the scramble is guaranteed ≠ the answer); tap bank words in order into the strip (tap a placed word to send it back), ✓ submits (string-compare); wrong KEEPS the words for fixing, an incomplete submit only nudges. 38-sentence bank, NO-REPEAT rotation; `make('sup'/'mulc')` → 1, `make('so')` → the bank. `aidsReveal:'always'`. |
**Bank expansion + single-answer review (2026-09).** The five reading banks grew
by 59 items (cloze 24→36, true/false 24→36, word-match 30→42 pairs,
sentence-order 24→38, rhyme 24→33; story-quiz stayed at 48 — its topic
rotation needs the four topics to hold EQUAL counts). Every candidate was passed
through three independent reviews — *is there a second defensible answer*, *is
the Hebrew and the niqqud right*, *can the answer be reached without reading* —
and 16 of 78 drafts were rejected. Repairs to entries that were already shipping:
a sentence with **locative inversion** (הַנְּסִיכָה גָּרָה בְּאַרמוֹן גָּדוֹל
also reads בְּאַרמוֹן גָּדוֹל גָּרָה הַנְּסִיכָה, and the string compare would reject
it) was replaced; two cloze gaps whose answer shared its ROOT with the verb
beside it (חָלַמְתִּי→חֲלוֹם, סִפְּרָה→סִפּוּר) had their verbs changed; and four
niqqud/grammar errors were fixed (לַחָלָל→לֶחָלָל ×2, הַמַּדָּעָנִים→הַמַּדְעָנִים,
הִבִּיט בּוֹ עַל→הִסְתַּכֵּל בַּ… דֶּרֶך). Three new tests pin the result:
...
**The banks are shared.** `problems.js _readingCards(m)` calls the SAME
`make(mode)` for the שפה language game, for Superman and for אַלּוּפָה, so an item
added to a bank is served by all three — pinned by
`test_new_reading_items_reach_superman_and_champion`, which builds 400 decks of
each and requires every probe item to turn up in both.

| `rhyme.ex.js`    | `TRH`         | `'rhy','lang','sup','mulc'` | Interactive **rhyme** (חָרוּז) — **phonological awareness**, the only language kind that is not reading-for-meaning: a cue word + picture (`{t:TRH,cue:{e,w},sound,opts:[{e,w}×3],a}`, `a` = 1-based correct option → num1) and 3 picture+word options of which **exactly one rhymes**; select → ✓ submits. The **33-pair bank** serves either member as the cue (50% flip) over a no-repeat shuffled PAIR queue. Unambiguity is **mechanical**: distractors come from a separate **20-word pool** filtered by the *normalized final PHONEME* (ן→נ, ף→פ, ם→מ…, and ך/ח collapsed to one sound — they are the same phoneme in Israeli Hebrew, so a ח-word could otherwise be offered against מֶלֶך/דֶּרֶך) so a distractor can never rhyme, and the two distractors differ from each other; pair-words/emoji are disjoint from the distractor pool. A wrong answer reveals the **SOUND AID** naming the ending ("🔊 הַמִּלָּה כַּד נִגְמֶרֶת בַּצְּלִיל **אַד**" — spelled as a syllable, not bare niqqud) then allows a re-pick. `make('sup'/'mulc'/'lang')` → 1, `make('rhy')` → all 33. `aidsReveal:'always'`. |
