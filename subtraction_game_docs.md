...
| `'br'` | גָּשֵׁר 10 🌈 | תרגילי גישור-10 ממוקדים — שתי סדרות פדגוגיות קבועות המוגשות לסירוגין (`_brTurn` נשמר ב-localStorage); סדרה 1: 25 תרגילי TA/TS, סדרה 2: 18 תרגילי TS/TA | 15 | 900 |
...
| `'sup'` | סוּפֶּרְמֶן 🦸 | משולב **מאוזן** — חלק שווה מכל סוג, 3 כ"א = 15 תרגילים: column_add (3), big_step (3 חיסורים), coin_mul (3), column_sub (6 = 3 ללא־פריטה + 3 עם־פריטה, כולם בטווח המלא `a∈[11,29]`) | 15 | 825 |
...
| `TBG` | `'big_step'`| `a ± b = ___` (מספר דו-ספרתי גדול, b=1–4 חיסור / 1–2 חיבור, ללא נשיאה/פריטה) | `a±b` |
...
> TM/TS/TA/TX/TZ/TW/TDA/TDS/TC/TT נבדקים דרך תיבת `#ans` הראשית; **TCA/TCS/TCM** מביאים DOM/CSS/בדיקה משלהם (`mount()`) ומורכבים ע"י core.js `_colxMount` לתוך `<div id="colx-root">`.

### תרגילים אינטראקטיביים עצמאיים (`exercises/<name>.ex.js`)

כל סוג תרגיל יושב בקובץ משלו ב-`exercises/` ורושם עצמו ל-`window.EXERCISES.types`. `EXERCISE_INDEX` (data.js) מקשר file→modes; ה-bg-loader מזריק רק את הקבצים שרמת המשחק מופיעה ב-`modes` שלהם, וה-recipes ב-`game/js/problems.js` בונים את הסשן מהטיפוסים הרשומים. `EXERCISE_OF_TYPE` ממפה `TCA→column_add`, `TCS→column_sub`, `TCM→coin_mul`.
...
חיסור הורחב לצעד 1–4 (היה 1–2); ללא נשיאה/פריטה ולעולם אינו חוצה את העשרת שמתחת (85−4 חוקי, 82−3 לעולם לא). `make`: `'big'` → 12 מאוזן חיסור/חיבור; `'mx'` → 2; `'sup'` → 3 חיסורי מספר גדול בלבד (`buildSubs`). `modes:['big','mx','sup']`.

#### `column_add.ex.js` — TCA (השתנה)

מגיע עד 48 (מחובר עליון 11–29, תחתון 2–19). `make('sup')` → `makePool(2,1)` = 3 בעיות (2 עם נשיאה + 1 בלי). `modes:['sup']`.
...
// ─── תרגיל נוכחי ───
let ptype;             // TM|TS|TA|TX|TZ|TW|TDA|TDS|TC|TT|TCA|TCS|TBG|TCM
