...
| `'mulc'` | אַלּוּפָה 🏆 (שכבת **קָשֶׁה**) | בְּרֵכָה **מְעֹרֶבֶת** (`_capPool` ל-20): כפל דו־שלבי (`mult_champ`/TMK, כפל עד 4) + הֶקֵּף מְצוּלָע (`perimeter`/TPP) + חיסור בטור מדורג (`column_sub`/TCS, מאוזן→טור, 20/15/10/0) + גְּרִירַת סִימָן הַשְׁוָואָה (`compare`/TCP) + בְּעָיוֹת מִלּוּלִיּוֹת (`word_prob`/TWP) + חִבּוּר שְׁלוֹשָׁה מִסְפָּרִים (`triple_sum`/TTS, `__+__+__=20`, בלי 0/10) | 20 | 900 |
...
const TM='missing',TS='sub',TA='add',TX='mixed',TZ='triple',TW='twin_sub',
      ...
      TTS='triple_sum';
const GL = 12;
// modePts(): mx→20, mulc→20, br/b20/sup→15, poly/mul/perim/cmp→15, big→10, אחרת mode||5
```
...
> TM/TS/TA/TX/TZ/TW/TDA/TDS/TC/TT נבדקים דרך תיבת `#ans` הראשית; **TCA/TCS/TCM/TBC/TPG/TMC/TMK/TPP/TCP/TWP/TTS** מביאים DOM/CSS/בדיקה משלהם (`mount()`) ומורכבים ע"י core.js `_colxMount` לתוך `<div id="colx-root">`.
