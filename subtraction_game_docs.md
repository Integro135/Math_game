...
|---|---|
| v6.40–v7.98 | **רסטרוקטורינג** — נקודת כניסה `index.html` + `game/` + `exercises/<name>.ex.js` + `backgrounds/*.bg.js`; המונוליט `subtraction_game.html` הוקפא; מיני-משחקי overlay/לבבות/חשבונייה הוסרו. **רמות חדשות בשכבת "בינוני"**: **סוּפֶּרְמֶן 🦸** (`sup`) — מאוזן, 3 מכל סוג (column_add / big_step חיסורים / coin_mul / column_sub) = 15; **חִסּוּר בְּטוּר ➖** (`sub_col`) — column_sub 12 + big_step 2. **טיפוסי תרגיל אינטראקטיביים חדשים**: `column_sub` (TCS, חיסור בטור עם פְּרִיטָה ושני מצבי borrow), `coin_mul` (TCM, ראשית הכפל עם מטבעות 5), `column_add` (TCA), `big_step` (TBG). `mx` הורחבה ל-19 תרגילים (כולל column_sub ללא־פריטה, big_step, tens, coins). יעדי פרס: `{br:900, mx:900, sup:825, sub_col:650}`. 5 מסכי הצלחה חדשים (phoenix-rising/peacock-fan/kaleidoscope-bloom/birthday-cake/carousel-spin). ראו סעיפים "רמות המשחק" + "סוגי תרגילים". |
...
בורר הרמות במודל ההגדרות מורכב מ-`DIFFICULTY_GROUPS` (`game/js/data.js`) בשתי שכבות: **קַל** (0/5/10/20) ו**בֵּינוֹנִי** (br/mx/sup/sub_col). תג הפרס 🎁 *אינו* חלק ב-`label` — הוא מתווסף בזמן הרינדור (`renderModePicker`) רק לרמות שיש להן יעד פרס פעיל, והיעד ניתן לעריכה לכל רמה ב-הגדרות.
...
| `'sup'` | סוּפֶּרְמֶן 🦸 | משולב **מאוזן** — חלק שווה מכל סוג, 3 כ"א = 15 תרגילים: column_add (3), big_step (3 חיסורים), coin_mul (3), column_sub (6 = 3 ללא־פריטה + 3 עם־פריטה) | 15 | 825 |
| `'sub_col'` | חִסּוּר בְּטוּר ➖ | משחק חיסור־בטור ייעודי: column_sub (12 תרגילים, ≥7 עם פריטה) + big_step (2) | 15 | 650 |

> **mode `'big'` (עַד 100 💯)** — הוסר מהבורר; תרגילי `big_step` (TBG ±1/2/3/4 על מספר דו-ספרתי גדול) חיים עכשיו **בתוך** מלכה, סופרמן וחיסור-בטור. הלוגיקה של `'big'` נשמרת כידית פנימית (מבחנים + הטיפוס המשותף TBG).
...
const GL = 12;
// modePts(): mx→20, br→15, sup→15, sub_col→15, big→10, אחרת mode||5
```
...
const DEFAULT_GIFT_GOALS={br:900, mx:900, sup:825, sub_col:650};
...
| `TCM` | `'coin_mul'`| ראשית הכפל: "כמה מטבעות 5 נכנסים ב-X" | `a/5` (כמות המטבעות) |
...
- **`make(mode)`**: `'sub_col'` → `makePool()` = 12 בעיות (a∈[11,29], b∈[2,19], a>b, ≥7 דורשות פריטה); `'mx'` (מלכה) → `makeNoBorrow(2)` (ללא פריטה, a≤19, אופרנדים ≤20); `'sup'` (סופרמן) → `makeSup()` = 3 ללא פריטה + 3 עם פריטה (=6). `modes:['sub_col','mx','sup']`.

#### `coin_mul.ex.js` — TCM ראשית הכפל

"כמה מטבעות של 5 נכנסים ב-X" — הכפל הראשון. בעיה `{t:TCM,a}` כאשר a הוא היעד (10/15/20); **התשובה היא a/5** (מספר המטבעות).
...
חיסור הורחב לצעד 1–4 (היה 1–2); ללא נשיאה/פריטה ולעולם אינו חוצה את העשרת שמתחת (85−4 חוקי, 82−3 לעולם לא). `make`: `'big'` → 12 מאוזן חיסור/חיבור; `'mx'` → 2; `'sup'` → 3 חיסורי מספר גדול בלבד (`buildSubs`); `'sub_col'` → 2. `modes:['big','mx','sup','sub_col']`.
...
| אירוע | שינוי |
|---|---|
| תשובה נכונה | `+modePts()` → 0→5, 5→5, 10→10, 20→20, mx→20, br/sup/sub_col→15, big→10 |
