...
| `'mulc'` | אַלּוּפָה 🏆 (שכבת **קָשֶׁה**) | בְּרֵכָה **מְעֹרֶבֶת** (`_capPool` ל-20): כפל דו־שלבי (`mult_champ`/TMK, כפל עד 4) + הֶקֵּף מְצוּלָע (`perimeter`/TPP) + חיסור בטור מדורג (`column_sub`/TCS, מאוזן→טור, 20/15/10/0) + גְּרִירַת סִימָן הַשְׁוָואָה (`compare`/TCP) + בְּעָיוֹת מִלּוּלִיּוֹת (`word_prob`/TWP) + חִבּוּר שְׁלוֹשָׁה מִסְפָּרִים (`triple_sum`/TTS, `__+__+__=20`, בלי 0/10) + כַּמָּה זֶה חֵצִי (`half`/THF, חלוקה שווה ל-2 — ראשית החילוק) + צַלָּחוֹת (`plates`/TPL, קבוצות שוות → סך הכל — סיפור הכפל) | 20 | 900 |
...
| `TTS` | `'triple_sum'`| **חִבּוּר שְׁלוֹשָׁה** — `__+__+__ = 20`, בוחרים שלושה מחוברים; **0 ו-10 אסורים** (תשובה נכונה־בסכום עם 0/10 מקבלת מחמאה, בלי קנס, ודורשת ניסיון אחר) | כל שלישייה שסכומה 20 (בלי 0/10) |
