...
> | Doc | Covers |
> |---|---|
| **Bulbasaur's eye lost its red again (2026-09, user: "בלבזור ברקע מלדיביים- העין שלו פתאום לא אדומה").** The red fill was already converted from a `radial-gradient(#df005d 100%,transparent)` to a plain `background-color` once, because that edge-case stop rasterises as TRANSPARENT on some tablet/mobile GPUs — but the WHITE GLINT was left as the very same pattern (`radial-gradient(1px 5px at center,#fff 100%,transparent)` as a `background-image` over the red), so a GPU that mishandles the stop floods the pupil with white instead of painting a 2×10px ellipse, and the eye reads white again. The glint is now a solid-filled `.pupil::after` (2×10px, the gradient's own ending shape): not one gradient left in that eye. Ruled out first, by measurement rather than by eye: the day-cycle tone the front canvas paints OVER the walkers (reddest pixel 221,0,92 → 187,20,101 from morning to night), the rendered size in the real game (eye 21–27 css px; red 27–31 % of the eye box at desktop/tablet/phone device scales), and the mirrored walking direction (red 58 % / 28 %) — the red survives all three here | **verified in the maldives tester and in the real game on the maldives theme, desktop/tablet/phone scales and both directions; no console errors** |
...
│  │  └─ form.css                  the exercise form's SHAPE (2026-09 redesign): header chips +
│  │                               progress rail, equation island, operator coins, keycap answer
...
| Gate | Result |
|---|---|
| **Exercise form REDESIGNED (2026-09, user: "עצב מחדש את טופס המשחק עצמו שמציג את התרגיל. שמור על הצורה הכללית אך הצע עיצוב חדש").** Same DOM, same places for every control; the new look is one stylesheet, `game/css/form.css`, loaded after `responsive.css` and BEFORE the runtime skin, so the 8 skins keep every colour and the file owns only shape + motion: a slim header (title 2rem, the two stats as CHIPS, the level as a chip, a full-width progress RAIL glowing in `--skin-accent`); the equation on an inner ISLAND; sticker numbers with a solid drop edge; operators as round COINS (`.eq-op` became a sized inline-flex circle — also inside the TZ sub-row and the shapes legend); the answer as a KEYCAP (100×78, `outline` ring, `placeholder="?"` added to the 5 input templates in `core.js`, placeholder colour `inherit` so the light unicorn box shows it, `ansBeckon` float while empty and idle — excluded on `.ans-ok/.ans-err` so the shake wins); the check button a coin that presses down; the hint a pill (`:empty` hidden); header/card corners flattened to 12px on the facing sides in all 8 skins so the panels dock. The aid buttons (games menu / digit hint, 40px now) sit in a reserved left GUTTER on the island (`padding-left` 112px, 64px via `:has()` when `#digit-hint-btn` is hidden) — the first pass had them landing on the check button on the narrow savanna/dubai columns. Phone (≤480): 58px boxes + 1.15em coins keep a three-box exercise on one line. FOLLOW-UP (user: "הרבה פעמים הגבול של ריבוע השווה קרוב מדי לתיבת הטקסט של התוצאה"): the keycap's outline ring reaches 6px past its border and takes NO layout space, so flex `gap` alone left the operator coin ~2px from the ring — the box now carries its own `margin-inline` (8px desktop / 5px phone, scoped to `.equation>.ans-inp` so the column-addition digit alignment in `.tx-eq-row` is untouched) and `.tz-sub-row` gap 5→9px. FOLLOW-UP 2 (user: "המרחק והגבהים של הגבולות מסביב לאופרטורים לא מיושרים ולא קבועים ברווחים ביניהם"): the coins (42px), numbers (57.6px line box) and box (78px) had different heights, so any TOP-aligned row (`.tz-inline`, the tens-crossing groups) put their centres 8–10px apart, and `.eq-n{min-width}` padded single digits so the gaps around a "6" were wider than around a "12". Now ONE AXIS / ONE GAP: every row item shares the numbers' margin-box height via `--eq-n` (coins get `margin-block:(--eq-n − coin)/2`, the box `(--eq-n − --eq-box)/2`, i.e. negative), the gap is `--eq-gap` (8px, phone 5px) on `.equation`, `.tz-inline`, `.tz-grp-top` AND `.tt-eq-wrap` (tens/hundreds, was 4px in aids.css), `min-width:0` on `.eq-n`; a group's sub-row is kept narrower than its top row ("+ 9" = 82px) with a 1.1rem coin + 50px sub box, and on the phone it takes `width:0;min-width:100%` so it spills instead of widening the group; the bond-line SVG (`viewBox 100×18`, 16px high) contributed an 89px intrinsic width and got the same `width:0;min-width:100%`. Measured by `align_probe.py` over six forced layouts (sub/add/missing/mixed/triple/hundreds): centre spread ≤1px everywhere, gaps 8/8/8/10 desktop and 5/5/5/6 phone (the +2 is the box's ring), was 15px spread and 8…24px gaps. FOLLOW-UP 3 (user: "תיבת הטקסט של התשובה חורגת מהמסגרת… וודא שהכל ממורכז ולא חורג"): two causes. (1) The equal-height scheme used the NUMBERS' height as the row height, so the box overhung its row by 10px each way (negative margins) and its ring came within 3px of the island frame — worse on the reef/maldives, whose skins set `.eq-row{padding:10px 14px}` over form.css's 18px. Now the row height is the BOX height (`--eq-box`: numbers and coins get positive `margin-block`, the box none — nothing overhangs) and the two skins lost their `.eq-row` padding (geometry is form.css's). (2) The aid buttons' in-island gutter (64/112px on the left) pushed the content 12–36px off the frame's centre; they were first moved to a tray hanging off the island's left edge (`margin-left:56px`) — which then left the FRAME itself off-centre in the card (user: "המסגרת עצמה של התרגיל לא ממורכזת"). Final: the buttons are ordinary flex items beside the check button (`.eq-mini-btns{position:static}`, overriding base.css's absolute parking, which was what let them collide with it), an inactive games button leaves the flow (`:not(.gm-active){display:none}`), and the island is full width with symmetric 18px padding — so flex centring puts the row's whole content on the frame's centre and the frame on the card's. A `grid-template-columns:1fr auto 1fr` variant that would have centred the exercise ALONE was tried and reverted: a `1fr` track cannot shrink below its content, so the buttons' column outgrew the empty one and threw the exercise 133px off-centre on the narrower savanna column. Probe over the six layouts: box clearance from the frame 12–14px (top/bottom), content centre offset 0px, frame centre offset 0px, no wrapping on the 700px columns | **verified by screenshots in the real game on reef/girls/dubai/savanna (1280×800), phone 390×844 (reef, girls), tablet 768×1024 (dubai); focus state + a wrong answer still paints the red border (`.ans-err`, rgb(255,82,82)); no console errors** |
| **Load-time swarm of blurry dots that then popped away (user: "when loading the space background first a lot of blurry pixels is being loaded that causes the loading to be slow, then they disapear and the backgroud becomes more responsive… within the first 10 seconds").** Diagnosed as two things acting together. (1) The GL sky's three star layers were gated on the tier's `lite` flag, so a downshift **deleted hundreds of stars**: the scene opened at `medium`, showed a swarm of blurry upscaled star dots while the device failed to hold that tier, then popped them all away 3-6 s later when the ladder dropped to `low`/`potato` — a quality change was changing *content*. The star field is now tier-independent and sparser (two layers, not three: the dense `N=190` speckle layer is deleted — it was mush at any scale below 1×); the bright stars were always the crisp 2-D layer's job, these only exist so the field streaks around the hole. (2) The scene opened at a fixed tier although the cost scales with the WINDOW's pixel count — a 1080p/4K window opened far heavier than a 720p one. `startTier()` now walks down from `medium` until `W·H·scale² ≤ PX_START` (650 k marched pixels), and `buildTargets` clamps any tier to `PX_MAX` (1.6 M). Also cut the load stall itself: the sky bake covers the frustum + a lensing margin (~2.4× the frame area), making that one pass the heaviest thing at load, so its density went 1.1 → `SKY_DENS = 0.85` texels per internal pixel (uPixAng, the star-AA footprint, tracks it) — with the deleted speckle layer that is ~2.3× less bake work | **verified with the 2-D layer hidden so only the GL sky is captured, across a medium → potato change: mean luminance 35.96 → 36.04, stddev 20.36 → 20.40 — the same sky, only softer (previously the star layers vanished outright); scene renders clean at the new sparser sky, no console errors** |
| **…and it was STILL "pixelated at some angles" — that part was not resolution at all (user: "why on some angles it is still pixelated? (since the blackhole is spinning)").** Measured first: rendering cost barely depends on the hole's orientation (1.18 / 1.13 / 1.10 fps at tilt 0 / 0.28 / 0.6 — a 7 % spread), so the ladder was *not* dropping a tier at particular angles. The real cause is undersampling of the **photon ring**: near the critical curve `b = 3√3/2 r_s` the lensing magnification diverges, so the ring is a band far narrower than a pixel carrying enormous brightness — one sample per pixel renders it as a **dotted chain of beads** (only pixels whose centre lands on the band light up), and which stretch of the ring is bright moves as the hole precesses, so the beading comes and goes with the orientation. No resolution fixes an infinitely thin feature; **pre-filtering** does. `main()` in both scene shaders now computes the pixel's own impact parameter, and for `b` between 0.93 and 1.35 × b_crit box-filters the pixel **radially** (the direction the ring's brightness varies fastest) with 3 marches, 2 on `lite` tiers — the trace body was refactored into `traceUV(uv)` for this. The gate comes from the ray itself, so it survives any zoom, and a box filter preserves the ring's energy | **measured before/after on the same frame: high-frequency beading along the ring **−45 %** in space2 (0.0210 → 0.0115) and **−41 %** in `blackhole.html` (0.0338 → 0.0194) with the ring's peak brightness unchanged (157.9 → 158.8 / 167.4 → 167.5); the annulus is ~2 % of the frame, costing +14 % frame time on the software renderer at the lowest tier (a wider first attempt cost +21 % for the same benefit, so it was narrowed); no seam at the annulus edge, no console errors** |
| **The black hole was permanently LOW-RES — a vsync bug in the adaptive-quality ladder (user: "try to make the blackhole less pixelated… lets make it one level up in the quality").** The climb rule was `emaMs < 11 ms`, but `requestAnimationFrame` is **vsync-locked**: a frame with plenty of GPU headroom still measures ~16.7 ms on a 60 Hz screen, so the gate was unreachable and `space2.bg.js` sat at its starting tier (0.42× internal resolution, CSS-upscaled = visibly blocky) forever, however fast the GPU was. Fixes: (1) the climb signal is now "holding the refresh interval" — `emaMs < 19` (a GPU that can't hold 60 Hz quantises to ~33 ms, well past the unchanged 27 ms downshift, so the rules can't overlap), with the post-downshift lockout raised 30 s → 45 s and a new `qFails`/`qCeil` guard so a tier that stalled twice is never climbed into again; (2) the ladder itself moved up a notch — `low` 0.42→**0.48**× (130 steps), `medium` 0.62→**0.72**× (220), `high` 0.82→**1.0**× (300), with `potato` (0.28×) left alone as the last-resort floor for weak devices; (3) the scene now **starts at `medium`** instead of `low`. `blackhole.html` got the same auto-gate fix and its default tier went `medium` → `high` | **verified headless: space2 starts at `medium` (922×518 for a 1280-wide window), the lensed star streaks and photon ring resolve sharply, and under software rendering the ladder still walks down (medium → low → potato) with no console errors; `blackhole.html` defaults to `high` (1050×590, 280 steps), no errors** |
...
| **Fixed word_match drag starting OFFSET from the press point on tablet (v9.75, user: "המשתמש לוחץ על המילה והגרירה מתחילה לא מהמקום שהמשתמש לחץ… בסטייה").** The `.wm-ghost` is `position:fixed; transform:translate(-50%,-50%)` at `left/top = clientX/clientY`, i.e. CENTRED on the finger, so grabbing a wide pill near its edge made it snap so its centre sat under the finger — a visible jump of ~half the pill width. Fix: capture the GRAB OFFSET `ox/oy` (press point minus the pill's centre) at `pointerdown`, and position the ghost at `clientX-ox, clientY-oy` in `onMove`, so the grabbed point stays exactly under the finger (no jump). Drop hit-test (`overPic`) still uses the finger position, so landing is unchanged. New test `test_word_match_drag_ghost_anchors_to_grab_point` grabs the widest pill near its right edge, drags by a known delta, and asserts the ghost centre = word-centre+delta (NOT the finger) — confirmed to FAIL pre-fix (741 vs 700, the 41px gap = the off-centre grab) via git-stash | **132 champion tests pass** |
