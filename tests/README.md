...
## מרוץ הטעינה — `enter_mode()`

`setMode(m)` מפעיל טעינה **א-סינכרונית** של קבצי התרגילים של המשחקון; כשהאחרון
נוחת, ה-callback **בונה מחדש את `problems` ומצייר את הכרטיס מחדש**. בדיקה שכופה
בעיה משלה לפני שזה קורה רואה אותה נמחקת רגע אחר כך — זה היה מקור ההבהוב
התלוי-סדר בחבילה (בכל הרצה נפלה בדיקת "loads and mounts" אחרת, כי מי שהפסיד
במרוץ תלוי בשאלה אם קובצי ה-`.ex.js` כבר היו ב-cache).

`core.js`/`main.js` מרימים `window.POOL_PENDING` לכל משך הבנייה.
**כל כניסה למשחקון חייבת לעבור דרך `enter_mode(page, m)`** (ב-`helpers.py`),
שממתין לדגל להתנקות. אל תקרא ל-`page.evaluate("setMode(...)")` ישירות לפני
כפיית בעיה. גם `solve_one()` ממתין לדגל.
